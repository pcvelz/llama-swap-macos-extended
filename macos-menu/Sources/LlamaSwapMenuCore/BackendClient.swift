...
public final class BackendClient: ObservableObject {
    ...
    private func handleEvent(_ data: String) {
        ...
        case "sessions":
            if let inner = envelope.data.data(using: .utf8),
               let snapshot = try? JSONDecoder().decode(SessionsSnapshot.self, from: inner) {
    ...
    /// The single place a "sessions" event becomes the menu's state: every
    /// row, plus the waiting counter, is copied straight from the contract
    /// (SessionStateContract.swift) with no client-side phase/context/rate
    /// computation. `queue.waiting`/`queue.byTier` satisfy the parity
    /// invariant (session-state-contract.md invariant 2) by construction -
    /// there is no second, independent count of PARKED rows here to disagree
    /// with it.
    ///
    /// `snapshot.cooldown` is intentionally NOT applied here: the dedicated
    /// "swapGrace" SSE event already drives `menuState.cooldown`, including
    /// the restart-detection logic above - a second writer of the same field
    /// from this event would race it for no benefit.
    private func applySessionsSnapshot(_ snapshot: SessionsSnapshot) {
        var state = menuState
