    ...
    const newModels = records
      ...
      .map((record): Model => {
        ...
        return {
          ...
          // /v1/models is the OpenAI-compatible listing used for Playground; it carries
          // no idle-TTL telemetry (that comes only from the modelStatus SSE / /api/models
          // path backing the `models` store). Default so Model stays fully populated.
          ttl: 0,
          lastUse: "",
          pinned: false,
          playgroundType,
