...
describe("pin/unpin model", () => {
  it("pins a model", async () => {
    vi.stubGlobal("fetch", vi.fn().mockResolvedValue({ ok: true }));
    await expect(pinModel("cq35")).resolves.toBeUndefined();
    expect(fetch).toHaveBeenCalledWith("/api/models/pin/cq35", { method: "POST" });
  });

  it("rejects when pinning fails", async () => {
    vi.stubGlobal("fetch", vi.fn().mockResolvedValue({ ok: false, status: 404 }));
    await expect(pinModel("cq35")).rejects.toThrow("Failed to pin model: 404");
  });

  it("unpins a model", async () => {
    vi.stubGlobal("fetch", vi.fn().mockResolvedValue({ ok: true }));
    await expect(unpinModel("cq35")).resolves.toBeUndefined();
    expect(fetch).toHaveBeenCalledWith("/api/models/unpin/cq35", { method: "POST" });
  });

  it("rejects when unpinning fails", async () => {
    vi.stubGlobal("fetch", vi.fn().mockResolvedValue({ ok: false, status: 404 }));
    await expect(unpinModel("cq35")).rejects.toThrow("Failed to unpin model: 404");
  });
});
