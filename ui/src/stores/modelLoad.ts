...
import { loadModel, pinModel, unloadSingleModel, unpinModel } from "./api";
...
/** Toggle a model's pin state (suppresses idle-TTL eviction while pinned). */
export async function handlePinToggle(m: Model): Promise<void> {
  try {
    if (m.pinned) {
      await unpinModel(m.id);
    } else {
      await pinModel(m.id);
    }
  } catch (e) {
