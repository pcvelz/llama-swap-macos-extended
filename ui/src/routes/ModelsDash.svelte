<script lang="ts">
  ...
  import ModelPinButton from "../components/ModelPinButton.svelte";
  ...
  import { getTTLLabel, modelServerPath } from "../lib/modelUtils";
  ...
  // Reactive clock -- ticks every second for the idle-TTL countdown badge.
...
{#snippet modelRow(model: Model)}
  <div class="hover:bg-muted/50 flex items-center gap-3 px-4 py-2.5">
    ...
    {#if $showCapabilityTags}
      ...
      {/if}
    {/if}
    {#if !model.peerID && model.state !== "stopped"}
      <span
        class="text-muted-foreground hidden text-xs tabular-nums sm:inline"
        title="TTL: {model.ttl}s, lastUse: {model.lastUse}"
      >{getTTLLabel(model, now)}</span>
    {/if}
